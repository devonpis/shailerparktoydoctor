...
| `tags` | Topic labels (**1–30** on the repair page); **3** chosen for Facebook/Instagram hashtags on social (see [`docs/publish-content-guards.md`](docs/publish-content-guards.md)) |
...
| `importance` | Optional — number for **home page highlights** (patient stories on `/`). **Higher = ranked higher** (lead story + up to six tiles). Omit or `null` = not shown there. Update with `node scripts/sync-home-highlights.mjs --set 0003=3` (see [`.cursor/rules/home-highlight-importance.mdc`](.cursor/rules/home-highlight-importance.mdc)). |
